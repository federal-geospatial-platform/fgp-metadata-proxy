...
class MyREST:
	...
	def get_data(self):
		...
		print "\nNumber of services for '%s': %s" % (self.root_url, len(services))

		if services is None: return None
		
		data_list = []
		
		if 'err' in services: return services

		# Cycle through each service
		print("\nGetting the data from the ArcGIS REST services for '%s'..." % self.root_url)
		#print "\nservices: %s" % services
		for idx, service in enumerate(services):
			print "Getting %s of %s services" % (idx + 1, 
					len(services))
	...
	def get_services(self, url=None, json=None):
		...
		# for idx, serv in enumerate(servs_json):
			...
			# services.append(self.get_service_json(url, serv))
