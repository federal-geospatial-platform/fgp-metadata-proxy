...
class PT_Extractor(main_ext.Extractor):
	...
	def __init__(self):
		...
		####################################################################
		# Create GeoDiscover page group
		
		geo_grp = main_ext.PageGroup('geodiscover', 'Alberta GeoDiscover')
		
		# Add arguments
		geo_grp.add_arg('word', title='Search Word')
		
		# Add URLs
		geo_grp.add_url('portal_url', 'https://geodiscover.alberta.ca/geoportal/rest/find/document')
		...
		srv_grp = main_ext.PageGroup('services', 'Alberta Map Services')
		
		# No arguments to add
		
		# Add URLs
		srv_grp.add_url('rest_url', 'https://maps.alberta.ca/genesis/rest/services')
		srv_grp.add_url('geocortex_url', 'https://maps.alberta.ca/Geocortex/Essentials/4.6/REST/sites')
		srv_grp.add_url('services2_url', 'https://services2.arcgis.com/jQV6VMr2Loovu7GU/arcgis/rest/services')
		srv_grp.add_url('tiles_url', 'https://tiles.arcgis.com/tiles/jQV6VMr2Loovu7GU/arcgis/rest/services')
		...
		open_grp = main_ext.PageGroup('opendata', 'Alberta Open Government')
		
		# Add arguments
		open_grp.add_arg('start', debug=True, title='Record Start')
		frm_arg = open_grp.add_arg('ds_type', title='Dataset Type')
		for format, lst in self.format_opts.items():
			format_name = lst[0]
			format_name = format_name.replace('+', ' ')
			format_name = format_name.replace('%2F', '/')
			format_name = format_name.replace('%2B', '+')
			format_name = format_name.replace('%28', '(')
			format_name = format_name.replace('%29', ')')
			frm_arg.add_opt(format_name, [format], lst)
		
		# Add URLs
		open_grp.add_url('main_url', 'https://open.alberta.ca/dataset')
		open_grp.add_url('api_url', 'https://open.alberta.ca/api/3/action/package_show')
	...
	def get_xml_data(self, in_xml): #, category, return_type='value'):
		...
		if lic_val == '0':
			...
			licence = shared.reduce_text(lic_val)
		else:
			licence = lic_val
	...
	def extract_opendata(self): #, word=None, format=None, start='0'):
		...
		for query_url in query_list:
			...
			for page in range(page_start, page_count):
				...
				for res in results:
					record += 1
					msg = "Extracting %s of approximately %s results from " \
							"'%s'" % (record, record_total, query_url)
					shared.print_oneliner(msg)
					
					# Get the dataset's link to get its ID
					...
					# Get the API link and JSON
					ds_api_url = "%s?id=%s" % (api_url, ds_name)
					ds_json = shared.get_json(ds_api_url)
					
					if not self.check_result(ds_json, ds_api_url, \
						"Alberta's Open Data Catalogue"): continue
					...
					try:
						...
						notes = ''
						usage = ''
						if 'notes' in res: notes = res['notes']
						if 'usage' in res: usage = res['usage']
						desc_str = '|'.join([notes, usage])
						
						# Get recent date
						recdate_str = ''
						if 'date_modified' in res:
							recdate_str = res['date_modified']
						
						# Get the start date
						startdate_str = ''
						if 'date_created' in res:
							startdate_str = res['date_created']
						
						# Get the update frequency
						freq_str = ''
						if 'updatefrequency' in res:
							freq_str = res['updatefrequency']
						...
						download_info = shared.get_download_text(formats, download_links)
						access_str, download_str = download_info.split('|')
						
						# Get the extents
						exts_str = ''
						if 'spatialcoverage1' in res:
							exts = res['spatialcoverage1']
							if exts is not None and not exts == '':
								...
								if len(exts_lst) == 4:
									...
									ext = [north, south, east, west]
									#print ext
									exts_str = shared.create_wkt_extents(ext)
									
						# If only including geospatial data
						if exts_str == '' and ds_url.find('geodiscover.alberta.ca') == -1: continue
						
						# Get the publisher
						organization = res['organization']
						publish_str = organization['title']
						
						pt_csv.add('Source', 'Alberta Open Data Catalogue')
						pt_csv.add('Title', title_str)
						pt_csv.add('Description', desc_str)
						pt_csv.add('Access', access_str)
						pt_csv.add('Extents', exts_str)
						pt_csv.add('Keywords', keywords_str)
						...
						pt_csv.add('Metadata URL', ds_api_url)
						pt_csv.add('Update Frequency', freq_str)
	...
	def extract_services(self):
		...
		rest_serv = services.PT_REST(rest_url)
		
		lyr_info = rest_serv.get_layers()
		
		if not self.check_result(lyr_info, rest_url, 'ArcGIS REST Map'): 
			return None
		...
		serv2_rest = services.PT_REST(services2_url)
		...
		tiles_rest = services.PT_REST(tiles_url)
