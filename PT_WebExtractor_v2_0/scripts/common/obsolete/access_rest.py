...
class MyREST:
	...
	def extract_data(self):
		service_data = self.get_services()
	...
	def get_service_data(self, service):
		...
		if isinstance(service, str):
			# If the service is a String, create a service for error
			#	checking.
		...
		if serv_name.find("Latest") == -1:
			...
			if 'layers' not in service:
				...
				if 'type' in service:
					serv_type = service['type']
					# if lyr_type.lower().find('group') > -1:
						# print lyr_type
						# print "Found the word 'group'"
						#answer = raw_input("Press enter...")
					if serv_type.lower() == 'group layer' or \
						serv_type.lower() == 'grouplayer':
						#print lyr_type
						#print "Now found full word."
						return None
				
				if title_str.find('Municipal') > -1:
					print "\nService Title: %s" % title_str
					print "Service Type: %s" % serv_type
					#answer = raw_input("Press enter...")
			...
			for idx, lyr in enumerate(layers):
				...
				if 'type' in lyr_json:
					...
					# if lyr_type.lower().find('group') > -1:
						...
						# print "Found the word 'group'"
						#answer = raw_input("Press enter...")
					if lyr_type.lower() == 'group layer' or \
						lyr_type.lower() == 'grouplayer':
						#print lyr_type
						#print "Now found full word."
						continue
				...
				# if title_str.find('Municipal') > -1:
					# print "\nLayer Title: %s" % title_str
					# print "Layer Type: %s" % lyr_type
					# answer = raw_input("Press enter...")
				...
				if 'parentLayer' in lyr_json and lyr_json['parentLayer'] is not None:
					...
					parent_name = parent_name.replace("(Group Layer)", "")
					parent_name = parent_name.replace("( Group Layer )", "")
	...
	def get_metadata(self, mdata_url):
		...
		if date is not None: mdata_dict['Recent Date'] = bsoup.get_text(date)
	...
	def get_services(self, url=None, json=None):
		...
		# If no 'services' in JSON and the URL contains 'MapServer',
		#	 end-of-the-line on a service
		if 'services' not in json and url.find('MapServer') > -1:
			...
			for serv in serv_data:
				services.append(serv)
			return services
		
		# If an error occurred in the JSON
		...
		for idx, serv in enumerate(servs_json):
			...
			# Ignore if it is the Latest service
			if serv_name.find('Latest') > -1: continue
			
			print "Extracting %s of %s services to list" \
					% (idx + 1, len(servs_json))
