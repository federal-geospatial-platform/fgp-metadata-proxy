...
class PT_Extractor(main_ext.Extractor):
	...
	def extract_catalogue(self): #, word=None, ds_type=None):
		...
		query_url = shared.get_post_query('%s/action/package_search' % api_url, params, 'ckan')
		
		# Use the API to get a list of results for the query URL
		print "query_url: %s" % query_url
		
		# Get the JSON results
		query_json = shared.get_json(query_url)
		
		# Get the total number of results from the count
		#	NOTE: Using rows=1000000 only returns 1000 records
		#			so the results will have to be looped
		total = query_json['result']['count']
		
		print "Number of results: %s" % total
		
		cur_idx = 1
		while cur_idx <= total:
		
			# Add the start parameter to params and query again
			params['start'] = cur_idx
			params['rows'] = '10000000'
			query_url = shared.get_post_query('%s/action/package_search' % api_url, params, 'ckan')
			
			# Get the results
			query_json = shared.get_json(query_url)
			results = query_json['result']['results']
			
			print "Number of sub-results for '%s': %s" % (len(results), query_url)
			...
			for res in results:
				...
				ds_mdata_url = "%s/%s" % (catal_url, ds_name)
				...
				try:
					...
					for r in resources:
						...
						if format == 'wms':
							...
							if res_url.find('arcgis') > -1:
								...
								wms = services.PT_WMS(res_url, True)
							...
							# Open the ArcGIS REST page
							arc_rest = services.PT_REST(res_url, silent=True)
					...
					if map_url.find('arcgis') > -1:
						...
						pre_info['Metadata URL'] = ds_mdata_url
						...
						pt_csv.add('Metadata URL', ds_mdata_url)
