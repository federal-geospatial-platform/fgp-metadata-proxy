...
REM ===========================================================================
SET NomApp=AB_RESOURCES_FORMATTER
SET fme=%FME2019%
...
REM First FME call, source data with nine wms resources (one with error), seven esri rest resources (one with error) and seven data sets with neither resource.  
set test_number=1
SET source=met\source%test_number%.ffs
set etalon=met\etalon%test_number%.ffs
set resultat=met\resultat.ffs
set lookup=met\ESRI_AB.csv
set log=met\log_%test_number%.log
set log_comp=met\log_comp_%test_number%.log

IF EXIST %log% del %log%
IF EXIST met\resultat.ffs DEL met\resultat.ffs
%fme% met\metrique_ab_resources_formatter.fmw ^
--IN_FFS_FILE %source% ^
