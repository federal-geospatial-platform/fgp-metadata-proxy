...
class PT_Extractor(main_ext.Extractor):
	...
	def get_xml_value(self, in_xml, tags):
	...
	def extract_geodiscover(self): #, word=None):
		...
		for idx, rec in enumerate(records):
			...
			mdata_type = self.get_xml_value(mdata_xml_soup, mdata_keys)

			# Get the horizontal datum
			hdatum = self.get_xml_value(mdata_xml_soup, 'horizdn')
			if hdatum == 'North American Datum of 1983':
				hdatum = 'NAD 83'
			ellipsoid = self.get_xml_value(mdata_xml_soup, 'ellips')
			if hdatum == '' and ellipsoid == '':
				refsys = mdata_xml_soup.find('referenceSystemIdentifier')
				code = self.get_xml_value(refsys, 'code')
				if not code == '':
					sp = "EPSG: %s" % code.replace("EPSG:", "")
				else:
					sp = ''
			else:
				sp = "%s %s" % (ellipsoid, hdatum)

			# Get the download URL
			download_keys = ['linkage', 'onlink']
			download_url = self.get_xml_value(mdata_xml_soup, download_keys)
			...
			if download_url.find('.aspx') > -1:
				...
				transferOptions = mdata_xml_soup.find('transferOptions')
				download_url = self.get_xml_value(transferOptions, 'linkage')
			...
			# Get the format
			# format_str = self.get_xml_value(mdata_xml_soup, 'formname')
			# if format_str == '':
				# md_format = mdata_xml_soup.find('MD_Format')
				# format_str = self.get_xml_value(md_format, 'name')
			# if format_str == '':
				# md_format = mdata_xml_soup.find('CI_OnlineResource')
				# format_str = self.get_xml_value(md_format, 'name')
				
			formats = []
			md_formats = mdata_xml_soup.find_all('MD_Format')
			...
			if len(formats) == 0:
				...
				resources = mdata_xml_soup.find_all('CI_OnlineResource')
			...
			# Get the data type
			dtype_keys = ['geoform', 'MD_SpatialRepresentationTypeCode']
			dtype = self.get_xml_value(mdata_xml_soup, dtype_keys)

			# Get the license
			licence_keys = ['fees', 'useLimitation']
			licence = self.get_xml_value(mdata_xml_soup, licence_keys)
			if licence == '0':
				licence = 'Free'
			elif len(licence) > 100:
				licence = shared.edit_description(licence)

			# Get the publisher
			pub_keys = ['deliveryPoint', 'publish', 'organisationName']
			pub_str = self.get_xml_value(mdata_xml_soup, pub_keys)
